...
class WideResNet(nn.Module):
    ...
    def dropblock(self, input, device = None, keep_prob = 0.9, block_size = 7):
        if keep_prob == 1:
            return input
        ...
        Msum = F.conv2d(M,
                        torch.ones((input.shape[1], 1, block_size, block_size)).to(device=device,
                                                                                             dtype=input.dtype),
    ...
    def forward(self, x, is_train = False, rand_index = 0, ratio = 0, is_featurecutmix = False, is_featureshift = False, is_featuremixup = False, is_dropblock = False, lam = 0, device = None):
        if is_train :
            out = self.conv1(x)
            out = self.block1(out)
            if is_dropblock:
